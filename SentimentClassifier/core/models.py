...
def get_optimizer(args, params):
    ...
    if args.algorithm == 'rmsprop':
        optimizer = optim.RMSprop(params, lr=args.rmsprop_learning_rate, alpha=0.9, eps=1e-06, weight_decay=0,
...
def load_embedding_reader(args):
    ...
    if args.emb_binary_path:
        from SentimentClassifier.core.w2vEmbReader import W2VEmbReader as EmbReader
        ...
        if args.emb_path:
            ...
            if (args.seed / 100 == 10):
                with open(args.out_dir_path + '/data/emb_reader_instance_' + str(args.emb_dim) + '.pkl',
                          'wb') as emb_data_file:
                    pk.dump(emb_reader,
                            emb_data_file)  # Note that saving an extremely big file/vocabulary will result in pickle memory error
...
class Net(nn.Module):
        ...
        if self.model_type == 'cnn' or self.model_type == 'crnn' or self.model_type == 'crcrnn':
            ...
            for i in range(args.cnn_layer):
                self.cnn.append(nn.Conv2d(in_channels=1,
                                          ...
                                          padding=(args.cnn_window_size // 2, 0))
                                # padding is on both sides, so padding=1 means it adds 1 on the left and 1 on the right
                                )
    ...
    def tensorLogger(self, method, result_tensor):
        ...
        if self.is_debug and self.batch_number == 0:
            self.log('%15s :' % layer_name, result_tensor)
